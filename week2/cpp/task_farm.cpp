...
void master(int nworker) {
  ...
  */
  ...
  auto n_tasks_per_worker = NTASKS / nworker;
  auto n_tasks_left = NTASKS % nworker;
  ...
  for (int worker = 1; worker <= nworker; worker++) {
    ...
    // post nonblocking receives, so we dont have unexpecting messages coming in
    // (at lest on master...)
    MPI_Request req;
    MPI_Irecv(result.data() + n_tasks_per_worker * (worker - 1), n_tasks,
              MPI_INT, worker, worker, MPI_COMM_WORLD, &req);
    requests.emplace_back(req);

    // we send the total number of tasks to the worker, contiguously
    MPI_Request send_req;
    MPI_Isend(&task[n_tasks_per_worker * (worker - 1)], n_tasks, MPI_INT,
              worker, worker, MPI_COMM_WORLD, &send_req);
  }
...
void worker(int rank) {
  ...
  */
  ...
  // first test for the size of the total number of tasks recevied
  int n_tasks;
  MPI_Status status;
  MPI_Probe(MASTER_ID, rank, MPI_COMM_WORLD, &status);
  MPI_Get_count(&status, MPI_INT, &n_tasks);

  std::vector<int> tasks(n_tasks);
  MPI_Recv(tasks.data(), n_tasks, MPI_INT, MASTER_ID, rank, MPI_COMM_WORLD,
           MPI_STATUS_IGNORE);

  for (int itask = 0; itask < n_tasks; itask++) {
    task_function(tasks[itask]);
    tasks[itask] = rank;
  }

  MPI_Send(tasks.data(), n_tasks, MPI_INT, MASTER_ID, rank, MPI_COMM_WORLD);
}
