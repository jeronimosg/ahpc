...
public:
  long nevents = 0;
  std::string name[8] = {"averageInteractionsPerCrossing",
                         "p_Rhad",
                         "p_Rhad1",
                         "p_TRTTrackOccupancy",
                         "p_topoetcone40",
  ...
  std::array<double, 8> means_sig{0},
      means_bckg{0}; // mean of signal and background for events
  std::array<double, 8>
      flip; // flip sign if background larger than signal for type of event
};
...
Data read_data() {
  ...
  while (std::getline(csvfile, line)) { // loop over lines until end of file
    ...
    // p_Rhad, p_Rhad1, p_TRTTrackOccupancy, p_topoetcone40, p_eTileGap3Cluster,
  ...
  for (long ev = 0; ev < ds.nevents; ev++) {
    ...
    if (ds.signal[ev]) {
      for (int i = 0; i < 8; i++)
        ds.means_sig[i] += ds.data[ev][i];
      nsig++;
    } else {
      for (int i = 0; i < 8; i++)
        ds.means_bckg[i] += ds.data[ev][i];
      nbckg++;
    }
  }
  for (int i = 0; i < 8; i++) {
...
// call this function to complete the task. It calculates the accuracy of a
...
double task_function(std::array<double, 8> &setting, Data &ds) {
  ...
  // accuracy is percentage of events that are predicted as true signal if and
  // only if a true signal
  double acc = 0;
  for (long ev = 0; ev < ds.nevents; ev++)
    acc += pred[ev] == ds.signal[ev];

  return acc / ds.nevents;
}

void master(int nworker, Data &ds) {
  std::array<std::array<double, 8>, n_cuts>
      ranges; // ranges for cuts to explore

  // loop over different event channels and set up cuts
  for (int i = 0; i < 8; i++)
    for (int j = 0; j < n_cuts; j++)
      ranges[j][i] =
          ds.means_sig[i] + j * (ds.means_bckg[i] - ds.means_sig[i]) / n_cuts;

  // generate list of all permutations of the cuts for each channel
  //   std::vector<std::array<double, 8>> settings(n_settings);
  // Flattened settings: store as a 1D vector of doubles, size n_settings*8
  std::vector<double> settings_flat(n_settings * 8);
  for (long k = 0; k < n_settings; k++) {
    long div = 1;
    for (int i = 0; i < 8; i++) {
      long idx = (k / div) % n_cuts;
      settings_flat[k * 8 + i] = ranges[idx][i];
      div *= n_cuts;
    }
  }
  ...
  The master should pass a set of settings to a worker, and the worker should
  return the accuracy
  */

  auto n_tasks_per_worker = n_settings / nworker;
  auto n_tasks_left = n_settings % nworker;

  for (int worker = 1; worker <= nworker; worker++) {
    int n_tasks = n_tasks_per_worker;
    if (worker <= n_tasks_left) {
      n_tasks++;
      n_tasks_left--;
    }

    MPI_Request req;
    MPI_Irecv(accuracy.data() + n_tasks_per_worker * (worker - 1), n_tasks,
              MPI_DOUBLE, worker, worker, MPI_COMM_WORLD, &req);
    requests.emplace_back(req);

    MPI_Request send_req;
    auto worker_settings =
        settings_flat.data() + n_tasks_per_worker * (worker - 1) * 8;
    MPI_Isend(worker_settings, n_tasks * 8, MPI_DOUBLE, worker, worker,
  ...
  std::cout << "Final cuts :\n";
  for (int i = 0; i < 8; i++)
    std::cout << std::setw(30) << ds.name[i] << " : "
              << settings_flat[idx_best * 8 + i] * ds.flip[i] << "\n";
...
void worker(int rank, Data &ds) {
  ...
  Use a call to "task_function" to complete a task and return accuracy to
  master.
  */
  MPI_Status status;
  int n_tasks;

  MPI_Probe(MASTER_ID, rank, MPI_COMM_WORLD, &status);
  MPI_Get_count(&status, MPI_DOUBLE, &n_tasks);

  int n_settings_received = n_tasks / 8;

  std::vector<double> tasks_flat(n_tasks);
  MPI_Recv(tasks_flat.data(), n_tasks, MPI_DOUBLE, MASTER_ID, rank,
           MPI_COMM_WORLD, MPI_STATUS_IGNORE);

  std::array<double, 8> worker_settings;
  std::vector<double> accuracy(n_settings_received);
  for (int itask = 0; itask < n_settings_received; itask++) {
    std::copy(tasks_flat.begin() + itask * 8,
              tasks_flat.begin() + (itask + 1) * 8, worker_settings.begin());
    accuracy[itask] = task_function(worker_settings, ds);
  }

  MPI_Send(accuracy.data(), n_settings_received, MPI_DOUBLE, MASTER_ID, rank,
           MPI_COMM_WORLD);
}
...
int main(int argc, char *argv[]) {
  ...
  if (rank == MASTER_ID)   // rank 0 is the master
